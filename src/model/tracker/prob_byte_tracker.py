...
class ProbabilisticByteTracker(ByteTracker, ProbabilisticTracker):  # type: ignore[misc]
    """Probabilistic ByteTracker with covariance support.

    Note: Method signatures intentionally differ from base class to support covariance matrices.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if det_score_mode not in self._score_modes:
            raise ValueError(f"Invalid det_score_mode: {det_score_mode}. Must be one of {self._score_modes}.")
        self.det_score_mode = det_score_mode
        self.compute_entropy = det_score_mode == "entropy" or self.with_primary_cascade or self.with_secondary_cascade
    ...
    def assign_ids(  # type: ignore[override]
        self,
        ids,
        det_bboxes,
        det_bbox_covs,
        ...
        Note: Signature differs from base class to support covariance matrices.
        ...
        for id in ids:
            track_mean, track_covariance = self.kf.project(
                self.tracks[id].mean, self.tracks[id].covariance, det_bbox_covs.cpu().numpy() if self.with_covariance else None
            )
            track_bboxes = np.concatenate((track_bboxes, track_mean[:4][None]), axis=0)
            if self.with_covariance:
                track_bbox_covs = np.concatenate((track_bbox_covs, track_covariance), axis=0)
        ...
        if mahalanobis:
            ...
            maha, threshold = self.mahalanobis(det_bboxes_rep[:, :4], det_bbox_covs_rep, track_bboxes, track_bbox_covs, one2one=True)
            ...
            det_entropy: Optional[torch.Tensor] = None
            if self.compute_entropy:
                det_entropy = self.get_covariance_entropy(det_bbox_covs)

            # ? Scale IOU with detection scores
            if weight_iou_with_det_scores and det_bboxes.numel() > 0:
                if score_mode == "entropy" and det_entropy is not None:
        ...
        track_labels = torch.tensor([self.tracks[id]["labels"][-1] for id in ids]).to(det_bboxes.device)
    ...
    def assign_ids_obs(  # type: ignore[override]
        ...
        # ? Compute distance
        if mahalanobis:
            maha, threshold = self.mahalanobis(det_bboxes[:, :4], det_bbox_covs, track_bboxes[:, :4], track_bbox_covs)
    ...
    def track(  # type: ignore[override]
        ...
        **_kwargs: Any,  # noqa: ARG002
        ...
        Returns:
            tuple: Tracking results (bboxes, bbox_covs, labels, ids)
        """
        ...
        if self.empty or bboxes.size(0) == 0:
            ...
            if self.init_ellipse_filter:
                valid_inds = self.check_width_height(img_metas, bboxes, bbox_covs, percent=self.init_percent)
                bboxes = bboxes[valid_inds]
                labels = labels[valid_inds]
                if bbox_covs is not None:
                    bbox_covs = bbox_covs[valid_inds]
            ...
            ids = torch.arange(self.num_tracks, self.num_tracks + num_new_tracks).to(labels)
            self.num_tracks += num_new_tracks
        else:
            # ? 0.0: Init ids
            ids = torch.full((bboxes.size(0),), -1, dtype=labels.dtype, device=labels.device)
            ...
            second_det_inds = (~first_det_inds) & (bboxes[:, -1] > self.obj_score_thrs["low"])
            ...
            if self.second_ellipse_filter:
                valid_inds = self.check_width_height(img_metas, second_det_bboxes, second_det_bbox_covs, percent=self.init_percent)
            ...
            first_match_track_inds, first_match_det_inds = self.assign_ids(
                ...
                False,
                self.with_primary_cascade,
                self.primary_cascade.get("num_bins", None) if self.primary_cascade else None,
            )
            # '-1' mean a detection box is not matched with tracklets in previous frame
            valid = first_match_det_inds > -1  # matched this round
            valid_torch = torch.from_numpy(valid).to(labels.device)
            first_det_ids[valid_torch] = torch.tensor(first_ids_to_match)[first_match_det_inds[valid]].to(labels)

            first_match_det_bboxes = first_det_bboxes[valid_torch]
            first_match_det_bbox_covs = first_det_bbox_covs[valid_torch]
            first_match_det_labels = first_det_labels[valid_torch]
            first_match_det_ids = first_det_ids[valid_torch]
            assert (first_match_det_ids > -1).all()

            first_unmatch_det_bboxes = first_det_bboxes[~valid_torch]
            first_unmatch_det_bbox_covs = first_det_bbox_covs[~valid_torch]
            first_unmatch_det_labels = first_det_labels[~valid_torch]
            first_unmatch_det_ids = first_det_ids[~valid_torch]
            ...
            valid_torch = torch.from_numpy(valid).to(labels.device)
            first_unmatch_det_ids[valid_torch] = torch.tensor(tentative_ids_to_match)[tentative_match_det_inds[valid]].to(labels)

            first_match_det_bboxes = torch.cat((first_match_det_bboxes, first_unmatch_det_bboxes[valid_torch]), dim=0)
            first_match_det_bbox_covs = torch.cat((first_match_det_bbox_covs, first_unmatch_det_bbox_covs[valid_torch]), dim=0)
            first_match_det_labels = torch.cat((first_match_det_labels, first_unmatch_det_labels[valid_torch]), dim=0)
            first_match_det_ids = torch.cat((first_match_det_ids, first_unmatch_det_ids[valid_torch]), dim=0)
            assert (first_match_det_ids > -1).all()

            first_unmatch_det_bboxes = first_unmatch_det_bboxes[~valid_torch]
            first_unmatch_det_bbox_covs = first_unmatch_det_bbox_covs[~valid_torch]
            first_unmatch_det_labels = first_unmatch_det_labels[~valid_torch]
            first_unmatch_det_ids = first_unmatch_det_ids[~valid_torch]
            assert (first_unmatch_det_ids == -1).all()
            ...
            _, second_match_det_inds = self.assign_ids(
                second_ids_to_match, second_det_bboxes, second_det_bbox_covs, second_det_labels, False, self.match_iou_thrs["low"], False, False, None
            )

            valid = second_match_det_inds > -1
            valid_torch = torch.from_numpy(valid).to(ids.device)
            second_det_ids[valid_torch] = torch.tensor(second_ids_to_match)[second_match_det_inds[valid]].to(ids)

            # ? 4. Gather all matched detections
            # * from second match, only matched detections are kept
            valid2 = second_det_ids > -1
            second_match_det_bboxes = second_det_bboxes[valid2]
            second_match_det_bbox_covs = second_det_bbox_covs[valid2]
            second_match_det_labels = second_det_labels[valid2]
            second_match_det_ids = second_det_ids[valid2]
            assert (second_match_det_ids > -1).all()

            # ? 4.1: remaining high score boxes are filtered using ellipses
            if self.final_ellipse_filter:
                valid_inds = self.check_width_height(img_metas, first_unmatch_det_bboxes, first_unmatch_det_bbox_covs, percent=self.final_percent)
            ...
            bboxes = torch.cat((first_match_det_bboxes, second_match_det_bboxes), dim=0)
            bbox_covs = torch.cat((first_match_det_bbox_covs, second_match_det_bbox_covs), dim=0)
            labels = torch.cat((first_match_det_labels, second_match_det_labels), dim=0)
            ids = torch.cat((first_match_det_ids, second_match_det_ids), dim=0)

            unmatched_track_ids = [
                id for id, _ in self.tracks.items() if id not in ids and (id in self.confirmed_ids or self.tracks[id].frame_ids[-1] == frame_id - 1)
            ]

            # ? Final matching
            if self.final_matching and len(unmatched_track_ids) > 0:
                # * Assume ellipse filter was applied before
                second_unmatch_det_bboxes = second_det_bboxes[~valid2]
                second_unmatch_det_bbox_covs = second_det_bbox_covs[~valid2]
                second_unmatch_det_labels = second_det_labels[~valid2]
                second_unmatch_det_ids = second_det_ids[~valid2]
                assert (second_unmatch_det_ids == -1).all()

                # ? Filter using ellipses before matching
                valid_inds = self.check_width_height(img_metas, second_unmatch_det_bboxes, second_unmatch_det_bbox_covs, percent=self.final_percent)
                ...
                if len(unmatch_det_ids) > 0:
                    ...
                    if self.expand_boxes:
                        unmatch_det_bboxes_exp, unmatch_det_bbox_covs_exp = self.get_outer_bboxes(unmatch_det_bboxes, unmatch_det_bbox_covs)
                        unmatch_track_bboxes, _ = self.get_outer_bboxes(unmatch_track_bboxes, unmatch_track_bbox_covs)
                    else:
                    ...
                    _, final_match_det_inds = self.assign_ids_obs(
                        ...
                        unmatch_track_bboxes,
                        unmatch_track_bbox_covs,
                        unmatch_det_bboxes_exp,
                        unmatch_det_bbox_covs_exp,
                        unmatch_det_labels,
                        False,
                        0.3,
                        self.use_mahalanobis,
                        self.with_secondary_cascade,
                        self.secondary_cascade.get("num_bins", None) if self.secondary_cascade else None,
                        iou_mode="giou",
                    )

                    valid = final_match_det_inds > -1
                    valid_torch = torch.from_numpy(valid).to(ids.device)
                    unmatch_det_ids[valid_torch] = torch.tensor(unmatched_track_ids)[final_match_det_inds[valid]].to(ids)

                    valid3 = unmatch_det_ids > -1
                    if self.return_expanded:
                        final_match_det_bboxes = unmatch_det_bboxes_exp[valid3]
                        final_match_det_bbox_covs = unmatch_det_bbox_covs_exp[valid3]
                    else:
                        final_match_det_bboxes = unmatch_det_bboxes[valid3]
                        final_match_det_bbox_covs = unmatch_det_bbox_covs[valid3]
                    final_match_det_labels = unmatch_det_labels[valid3]
                    final_match_det_ids = unmatch_det_ids[valid3]
                    assert (final_match_det_ids > -1).all()

                    n_first = len(first_unmatch_det_ids)
                    if self.return_remaining_expanded:
                        first_unmatch_det_bboxes = unmatch_det_bboxes[:n_first][~valid3[:n_first]]
                        first_unmatch_det_bbox_covs = unmatch_det_bbox_covs[:n_first][~valid3[:n_first]]
                        first_unmatch_det_labels = unmatch_det_labels[:n_first][~valid3[:n_first]]
                        first_unmatch_det_ids = unmatch_det_ids[:n_first][~valid3[:n_first]]
                    else:
                        first_unmatch_det_bboxes = first_unmatch_det_bboxes[~valid3[: len(first_unmatch_det_bboxes)]]
                        first_unmatch_det_bbox_covs = first_unmatch_det_bbox_covs[~valid3[: len(first_unmatch_det_bbox_covs)]]
                        first_unmatch_det_labels = first_unmatch_det_labels[~valid3[: len(first_unmatch_det_labels)]]
                        first_unmatch_det_ids = first_unmatch_det_ids[~valid3[:n_first]]
                ...
                if self.return_remaining_expanded and len(first_unmatch_det_ids) > 0:
                    first_unmatch_det_bboxes, first_unmatch_det_bbox_covs = self.get_outer_bboxes(
            ...
            ids[new_track_inds] = torch.arange(self.num_tracks, self.num_tracks + num_new).to(labels)
