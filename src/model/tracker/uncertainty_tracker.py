...
class UncertaintyTracker(OCSORTTracker, ProbabilisticTracker):
    ...
    def state_association(self, ids, det_bboxes, det_bbox_covs, det_labels, weight_iou_with_det_scores=False, match_iou_thr=0.5, mode="iou"):
        ...
        if weight_iou_with_det_scores and det_bboxes.numel() > 0:
            if self.det_score_mode == "entropy" and det_entropy is not None:
        ...
        track_labels = torch.tensor([self.tracks[id]["labels"][-1] for id in ids]).to(det_labels)
    ...
    def detection_matching(
        ...
        if self.custom_secondary:
            ...
            custom_dists = self.secondary_distance(track_bboxes[:, :4], track_bbox_covs_diag, det_bboxes[:, :4], det_bbox_covs_diag)
            ...
            if self.compute_entropy and det_bbox_covs is not None:
                det_entropy = self.get_covariance_entropy(det_bbox_covs)
        ...
        # ? Solve the linear assignment problem
        if dists is not None and threshold is not None and dists.size > 0:
            if cascade:
                if det_entropy is None:
                    raise ValueError("det_entropy is required for cascade matching.")
    ...
    def track(self, img, img_metas, model, bboxes, labels, frame_id, rescale=False, **kwargs):  # type: ignore[override]  # noqa: ARG002
        ...
        if self.empty or bboxes.size(0) == 0:
            ...
            _first_match_track_inds, first_match_det_inds, _first_track_bboxes = self.state_association(
                first_ids_to_match, det_bboxes, det_bbox_covs, det_labels, self.weight_iou_with_det_scores, self.match_iou_thr
            )
            # '-1' mean a detection box is not matched with tracklets in previous frame
            matched_np = first_match_det_inds > -1  # * matched this round
            matched_torch = torch.from_numpy(matched_np).to(labels.device)
            det_ids[matched_torch] = torch.tensor(first_ids_to_match)[first_match_det_inds[matched_np]].to(labels)

            first_match_det_bboxes = det_bboxes[matched_torch]
            first_match_det_bbox_covs = det_bbox_covs[matched_torch]
            first_match_det_labels = det_labels[matched_torch]
            first_match_det_ids = det_ids[matched_torch]
            assert (first_match_det_ids > -1).all()

            first_unmatch_det_bboxes = det_bboxes[~matched_torch]
            first_unmatch_det_bbox_covs = det_bbox_covs[~matched_torch]
            first_unmatch_det_labels = det_labels[~matched_torch]
            first_unmatch_det_ids = det_ids[~matched_torch]
            ...
            _second_match_track_inds, second_match_det_inds, _second_track_bboxes = self.state_association(
                second_ids_to_match,
            ...
            matched_np = second_match_det_inds > -1
            matched_torch = torch.from_numpy(matched_np).to(labels.device)
            first_unmatch_det_ids[matched_torch] = torch.tensor(second_ids_to_match)[second_match_det_inds[matched_np]].to(labels)

            second_match_det_bboxes = first_unmatch_det_bboxes[matched_torch]
            second_match_det_bbox_covs = first_unmatch_det_bbox_covs[matched_torch]
            second_match_det_labels = first_unmatch_det_labels[matched_torch]
            second_match_det_ids = first_unmatch_det_ids[matched_torch]
            ...
            match_det_bboxes = torch.cat((first_match_det_bboxes, second_match_det_bboxes), dim=0)
            match_det_bbox_covs = torch.cat((first_match_det_bbox_covs, second_match_det_bbox_covs), dim=0)
            match_det_labels = torch.cat((first_match_det_labels, second_match_det_labels), dim=0)
            match_det_ids = torch.cat((first_match_det_ids, second_match_det_ids), dim=0)
            assert (match_det_ids > -1).all()

            # * Remaining unmatched detections after step 1-2
            unmatch_det_bboxes = first_unmatch_det_bboxes[~matched_torch]
            unmatch_det_bbox_covs = first_unmatch_det_bbox_covs[~matched_torch]
            unmatch_det_labels = first_unmatch_det_labels[~matched_torch]
            unmatch_det_ids = first_unmatch_det_ids[~matched_torch]
            assert (unmatch_det_ids == -1).all()

            all_track_ids = [id for id, _ in self.tracks.items()]
            unmatched_track_ids = torch.tensor(
                [id for id in all_track_ids if id not in match_det_ids and (id in self.confirmed_ids or self.tracks[id].tracked)]
            )
            ...
            if len(unmatched_track_ids) > 0:
                ...
                third_match_track_inds, third_match_det_inds, _third_track_bboxes = self.detection_matching(
                    unmatched_track_ids.tolist(),
                    last_observations,
                    last_observation_covs,
                    unmatch_det_bboxes,
                ...
                matched_np = third_match_det_inds > -1
                matched_torch = torch.from_numpy(matched_np).to(labels.device)
                remain_det_ids[matched_torch] = unmatched_track_ids.clone()[third_match_det_inds[matched_np]].to(labels)

                third_match_det_bboxes = unmatch_det_bboxes[matched_torch]
                third_match_det_bbox_covs = unmatch_det_bbox_covs[matched_torch]
                third_match_det_labels = unmatch_det_labels[matched_torch]
                third_match_det_ids = remain_det_ids[matched_torch]
                ...
                third_unmatch_det_bboxes = unmatch_det_bboxes[~matched_torch]
                third_unmatch_det_bbox_covs = unmatch_det_bbox_covs[~matched_torch]
                third_unmatch_det_labels = unmatch_det_labels[~matched_torch]
                third_unmatch_det_ids = remain_det_ids[~matched_torch]
                ...
                match_det_bboxes = torch.cat((match_det_bboxes, third_match_det_bboxes), dim=0)
                match_det_bbox_covs = torch.cat((match_det_bbox_covs, third_match_det_bbox_covs), dim=0)
                match_det_labels = torch.cat((match_det_labels, third_match_det_labels), dim=0)
                match_det_ids = torch.cat((match_det_ids, third_match_det_ids), dim=0)
            ...
            if self.ellipse_filter:
                valid_inds = self.check_width_height(img_metas, unmatch_det_bboxes, unmatch_det_bbox_covs, percent=self.percent)
            ...
            if self.expand_boxes and unmatch_det_bboxes.numel() > 0:
                ...
                if len(unmatched_track_ids) > 0:
                    ...
                    matched_np = fourth_match_det_inds > -1
                    matched_torch = torch.from_numpy(matched_np).to(labels.device)
                    remain_det_ids[matched_torch] = unmatched_track_ids.clone()[fourth_match_det_inds[matched_np]].to(labels)

                    fourth_match_det_bboxes = unmatch_det_bboxes[matched_torch]
                    fourth_match_det_bbox_covs = unmatch_det_bbox_covs[matched_torch]
                    fourth_match_det_labels = unmatch_det_labels[matched_torch]
                    fourth_match_det_ids = remain_det_ids[matched_torch]
                    assert (fourth_match_det_ids > -1).all()

                    fourth_unmatch_det_bboxes = unmatch_det_bboxes[~matched_torch]
                    fourth_unmatch_det_bbox_covs = unmatch_det_bbox_covs[~matched_torch]
                    fourth_unmatch_det_labels = unmatch_det_labels[~matched_torch]
                    fourth_unmatch_det_ids = remain_det_ids[~matched_torch]
                    ...
                    match_det_bboxes = torch.cat((match_det_bboxes, fourth_match_det_bboxes), dim=0)
                    match_det_bbox_covs = torch.cat((match_det_bbox_covs, fourth_match_det_bbox_covs), dim=0)
                    match_det_labels = torch.cat((match_det_labels, fourth_match_det_labels), dim=0)
                    match_det_ids = torch.cat((match_det_ids, fourth_match_det_ids), dim=0)
