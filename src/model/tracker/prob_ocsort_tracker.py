...
class ProbabilisticOCSORTTracker(OCSORTTracker, ProbabilisticTracker):  # type: ignore[misc]
    """Probabilistic OCSORTTracker with covariance support.
    ...
    def init_track(self, id, obj):
        ...
        self.tracks[id].velocity = torch.tensor((-1, -1)).to(obj[bbox_id].device)  # placeholder
    ...
    def update_track(self, id, obj):
        ...
        bbox2 = obj[bbox_id]
        self.tracks[id].velocity = self.vel_direction(bbox1, bbox2).to(obj[bbox_id].device)

    def ocm_assign_ids(  # type: ignore[override]
        self,
        ids,
        det_bboxes,
        det_bbox_covs,
        ...
        for id in ids:
            ...
            track_bboxes = np.concatenate((track_bboxes, track_mean[None]), axis=0)
        ...
        if len(ids) > 0 and len(det_bboxes) > 0:
            track_velocities = torch.stack([self.tracks[id].velocity for id in ids]).to(det_bboxes.device)
            k_step_observations = torch.stack(
                [cast(torch.Tensor, self.k_step_observation(self.tracks[id])) for id in ids]
            ...
            vel_to_match = self.vel_direction_batch(k_step_observations[:, :4], det_bboxes[:, :4])
            track_velocities = track_velocities[:, None, :].repeat(1, det_bboxes.shape[0], 1)
    ...
    def ocr_assign_ids(  # type: ignore[override]
        self,
        track_obs,
        track_obs_covs,
        det_bboxes,
        ...
        # ? Compute distance
        if mahalanobis:
            maha, threshold = self.mahalanobis(det_bboxes[:, :4], det_bbox_covs, track_obs[:, :4], track_obs_covs)
    ...
    def track(  # type: ignore[override]
        ...
        **_kwargs: Any,  # noqa: ARG002
        ...
        if self.empty or bboxes.size(0) == 0:
            ...
            if self.init_ellipse_filter:
                valid_inds = self.check_width_height(img_metas, det_bboxes, det_bbox_covs, percent=self.init_percent)
            ...
            _, first_match_det_inds = self.ocm_assign_ids(
                first_ids_to_match, det_bboxes, det_bbox_covs, self.weight_iou_with_det_scores, self.match_iou_thr
            )
            ...
            det_ids[valid_torch] = torch.tensor(first_ids_to_match)[first_match_det_inds[valid]].to(labels)

            first_match_det_bboxes = det_bboxes[valid_torch]
            first_match_det_bbox_covs = det_bbox_covs[valid_torch]
            first_match_det_labels = det_labels[valid_torch]
            first_match_det_ids = det_ids[valid_torch]
            assert (first_match_det_ids > -1).all()

            first_unmatch_det_bboxes = det_bboxes[~valid_torch]
            first_unmatch_det_bbox_covs = det_bbox_covs[~valid_torch]
            first_unmatch_det_labels = det_labels[~valid_torch]
            first_unmatch_det_ids = det_ids[~valid_torch]
            ...
            _, tentative_match_det_inds = self.ocm_assign_ids(
                tentative_ids_to_match, first_unmatch_det_bboxes, first_unmatch_det_bbox_covs, self.weight_iou_with_det_scores, self.match_iou_thr
            )
            valid_np = tentative_match_det_inds > -1
            valid_torch = torch.from_numpy(valid_np).to(labels.device)
            first_unmatch_det_ids[valid_torch] = torch.tensor(tentative_ids_to_match)[tentative_match_det_inds[valid_np]].to(labels)

            match_det_bboxes = torch.cat((first_match_det_bboxes, first_unmatch_det_bboxes[valid_torch]), dim=0)
            match_det_bbox_covs = torch.cat((first_match_det_bbox_covs, first_unmatch_det_bbox_covs[valid_torch]), dim=0)
            match_det_labels = torch.cat((first_match_det_labels, first_unmatch_det_labels[valid_torch]), dim=0)
            match_det_ids = torch.cat((first_match_det_ids, first_unmatch_det_ids[valid_torch]), dim=0)
            assert (match_det_ids > -1).all()

            # * Remaining unmatched detections after step 1-2
            unmatch_det_bboxes = first_unmatch_det_bboxes[~valid_torch]
            unmatch_det_bbox_covs = first_unmatch_det_bbox_covs[~valid_torch]
            unmatch_det_labels = first_unmatch_det_labels[~valid_torch]
            unmatch_det_ids = first_unmatch_det_ids[~valid_torch]
            assert (unmatch_det_ids == -1).all()

            all_track_ids = [id for id, _ in self.tracks.items()]
            unmatched_track_ids = torch.tensor([ind for ind in all_track_ids if ind not in match_det_ids])

            unmatched_track_bboxes = unmatch_det_bboxes.new_zeros((0, 4))
            unmatched_track_bbox_covs = unmatch_det_bbox_covs.new_zeros((0, 4, 4))
            ...
            if len(unmatched_track_ids) > 0:
                ...
                remain_det_ids = torch.full((unmatch_det_bboxes.size(0),), -1, dtype=labels.dtype, device=labels.device)
                ...
                valid_np = ocr_match_det_inds > -1
                valid_torch = torch.from_numpy(valid_np).to(labels.device)
                remain_det_ids[valid_torch] = unmatched_track_ids.clone()[ocr_match_det_inds[valid_np]].to(labels)

                ocr_match_det_bboxes = unmatch_det_bboxes[valid_torch]
                ocr_match_det_bbox_covs = unmatch_det_bbox_covs[valid_torch]
                ocr_match_det_labels = unmatch_det_labels[valid_torch]
                ocr_match_det_ids = remain_det_ids[valid_torch]
                assert (ocr_match_det_ids > -1).all()

                ocr_unmatch_det_bboxes = unmatch_det_bboxes[~valid_torch]
                ocr_unmatch_det_bbox_covs = unmatch_det_bbox_covs[~valid_torch]
                ocr_unmatch_det_labels = unmatch_det_labels[~valid_torch]
                ocr_unmatch_det_ids = remain_det_ids[~valid_torch]
                assert (ocr_unmatch_det_ids == -1).all()

                match_det_bboxes = torch.cat((match_det_bboxes, ocr_match_det_bboxes), dim=0)
                match_det_bbox_covs = torch.cat((match_det_bbox_covs, ocr_match_det_bbox_covs), dim=0)
                match_det_labels = torch.cat((match_det_labels, ocr_match_det_labels), dim=0)
            ...
            if self.final_ellipse_filter:
                valid_inds = self.check_width_height(img_metas, unmatch_det_bboxes, unmatch_det_bbox_covs, percent=self.final_percent)
            ...
            if self.final_matching and len(unmatched_track_ids) > 0:
                if len(unmatch_det_ids) > 0:
                    if self.expand_boxes:
                        ...
                        unmatched_track_bboxes, _ = self.get_outer_bboxes(unmatched_track_bboxes, unmatched_track_bbox_covs)
                    else:
                    ...
                    _, final_match_det_inds = self.ocr_assign_ids(
                        ...
                        (self.secondary_cascade or {}).get("num_bins", None),
                        iou_mode="giou",
                    )

                    valid_np = final_match_det_inds > -1
                    valid_torch = torch.from_numpy(valid_np).to(labels.device)
                    remain_det_ids[valid_torch] = unmatched_track_ids.clone()[final_match_det_inds[valid_np]].to(labels)

                    final_match_det_bboxes = unmatch_det_bboxes[valid_torch]
                    final_match_det_bbox_covs = unmatch_det_bbox_covs[valid_torch]
                    final_match_det_labels = unmatch_det_labels[valid_torch]
                    final_match_det_ids = remain_det_ids[valid_torch]
                    assert (final_match_det_ids > -1).all()

                    unmatch_det_bboxes = unmatch_det_bboxes[~valid_torch]
                    unmatch_det_bbox_covs = unmatch_det_bbox_covs[~valid_torch]
                    unmatch_det_labels = unmatch_det_labels[~valid_torch]
                    unmatch_det_ids = remain_det_ids[~valid_torch]
                    assert (unmatch_det_ids == -1).all()

                    match_det_bboxes = torch.cat((match_det_bboxes, final_match_det_bboxes), dim=0)
                    match_det_bbox_covs = torch.cat((match_det_bbox_covs, final_match_det_bbox_covs), dim=0)
                    match_det_labels = torch.cat((match_det_labels, final_match_det_labels), dim=0)
                    match_det_ids = torch.cat((match_det_ids, final_match_det_ids), dim=0)
            else:
                if self.return_remaining_expanded:
                    unmatch_det_bboxes, unmatch_det_bbox_covs = self.get_outer_bboxes(unmatch_det_bboxes, unmatch_det_bbox_covs)
            ...
            bbox_covs = torch.cat((match_det_bbox_covs, unmatch_det_bbox_covs), dim=0)
