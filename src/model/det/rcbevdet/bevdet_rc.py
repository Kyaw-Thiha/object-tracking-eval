...
class RadarConvFuser(nn.Module):
    def __init__(self, in_channels: Sequence[int], out_channels: int, deconv_blocks: int) -> None:
        ...
        deconv = []
        deconv_in = [sum(self.in_channels) + out_channels]
    ...
    def init_weights(self):
        ...
        for module in self.deconv:
            normal_init(module, mean=0, std=0.001)
...
class BEVDet_RC(CenterPoint):
    ...
    def image_encoder(self, img, stereo=False):
        ...
        if self.img_backbone is None:
            raise RuntimeError('img_backbone is missing.')
        ...
        if self.with_img_neck:
            if self.img_neck is None:
                raise RuntimeError('img_neck is missing.')
            x = self.img_neck(x)
    ...
    def bev_encoder(self, x):
        if hasattr(self, 'img_down2top_encoder_backbone'):
            down2top = getattr(self, 'img_down2top_encoder_backbone')
            if callable(down2top):
                x = down2top(x)
    ...
    def extract_feat(self, points, img, img_metas, radar, gt_bboxes_3d = None , **kwargs):  #add gt
        ...
        for i in range(0,len(img_feats)):
            fusion_f = img_feats[i]

            if hasattr(self, 'DeformAttn1') and self.DeformAttn1 is not None:
                radar_feat = self.radar_reduc_conv(radar_feats[i])
                radar_feat = rearrange(radar_feat, 'b c h w -> b (h w) c')
                img_feat = rearrange(img_feats[i], 'b c h w -> b (h w) c')
                
                device = img_feats[i].device
                mask = torch.zeros(1, 1, self.bev_size, self.bev_size, device=device)
                ...
                fusion_f1 = self.DeformAttn1(query=self.with_pos_embed(radar_feat, pos1), 
                                             ...
                                             input_flatten = self.with_pos_embed(img_feat, pos2), 
                ...
                fusion_f2 = self.DeformAttn2(query=self.with_pos_embed(img_feat, pos2), 
                                             reference_points = reference_point2, 
                                             input_flatten = self.with_pos_embed(radar_feat, pos1), 
                                             input_spatial_shapes=torch.tensor([(self.bev_size, self.bev_size)]).to(device), 
                                             input_level_start_index=torch.tensor([0, self.bev_size*self.bev_size]).to(device) , 
                                             input_padding_mask=None)
    ...
    def forward_test(self,
        ...
        """
        if img_inputs is None or img_metas is None:
            raise ValueError('img_inputs and img_metas must not be None.')
        if radar is None:
    ...
    def forward_pts_train(self,  # type: ignore[override]
                          pts_feats,
                          gt_bboxes_3d,
                          gt_labels_3d,
                          gt_masks_bev,
    ...
    def aug_test(self, points, img_metas, img=None, rescale=False):  # type: ignore[override]
...
class BEVDet4D_RC(BEVDet_RC):
    ...
    def init_weights(self):
        ...
        super_obj = cast(Any, super(BEVDet4D_RC, self))
        super_obj.init_weights()
        if self.freeze_img:
            
            if self.with_img_backbone:
                assert self.img_backbone is not None
                for param in self.img_backbone.parameters():
                    param.requires_grad = False
            if self.with_img_neck:
                assert self.img_neck is not None
    ...
    def prepare_inputs(self, inputs, stereo=False):  # type: ignore[override]
        # split the inputs into each frame
        B, N, C, H, W = inputs[0].shape
        N = N // self.num_frame
        ...
        curr2adjsensor: Optional[list[Optional[torch.Tensor]]] = None
        if stereo:
            temporal_frame_raw = getattr(self, 'temporal_frame')
            extra_ref_frames_raw = getattr(self, 'extra_ref_frames')
            if not isinstance(temporal_frame_raw, int):
                raise TypeError('temporal_frame must be int.')
            if not isinstance(extra_ref_frames_raw, int):
                raise TypeError('extra_ref_frames must be int.')
            temporal_frame = temporal_frame_raw
            extra_ref_frames = extra_ref_frames_raw
            sensor2egos_cv, ego2globals_cv = sensor2egos, ego2globals
            sensor2egos_curr = \
                sensor2egos_cv[:, :temporal_frame, ...].double()
            ego2globals_curr = \
                ego2globals_cv[:, :temporal_frame, ...].double()
            sensor2egos_adj = \
                sensor2egos_cv[:, 1:temporal_frame + 1, ...].double()
            ego2globals_adj = \
                ego2globals_cv[:, 1:temporal_frame + 1, ...].double()
            curr2adjsensor_tensor = \
                torch.inverse(ego2globals_adj @ sensor2egos_adj) \
                @ ego2globals_curr @ sensor2egos_curr
            curr2adjsensor_t = curr2adjsensor_tensor.float()
            curr2adjsensor_split = torch.split(curr2adjsensor_t, 1, 1)
            curr2adjsensor_list: list[Optional[torch.Tensor]] = [
                p.squeeze(1) for p in curr2adjsensor_split
            ]
            curr2adjsensor_list.extend([None for _ in range(extra_ref_frames)])
            curr2adjsensor = curr2adjsensor_list
    ...
    def extract_img_feat(self,  # type: ignore[override]
                         img,
                         img_metas,
                         pred_prev=False,
                         sequential=False,
...
class BEVDepth4D_RC(BEVDet4D_RC):
    ...
    def forward_train(self,  # type: ignore[override]
                      points=None,
                      img_metas=None,
                      radar=None,
                      gt_bboxes_3d=None,
...
class BEVStereo4D_RC(BEVDepth4D_RC):
    ...
    def prepare_bev_feat(self, img, sensor2keyego, ego2global, intrin,  # type: ignore[override]
    ...
    def extract_img_feat(self,  # type: ignore[override]
        ...
        assert curr2adjsensor is not None
        """Extract features of images."""
        bev_feat_list = []
