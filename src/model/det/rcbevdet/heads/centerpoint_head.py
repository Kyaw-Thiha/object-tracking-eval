...
from ..core.box3d_nms import circle_nms, nms_bev, oval_nms
from ..core.bbox_utils import xywhr2xyxyr
from ..core.clip_sigmoid import clip_sigmoid
from ..core.gaussian import draw_heatmap_gaussian, gaussian_radius
...
class CenterHead(BaseModule):
    ...
    def loss(self, gt_bboxes_3d, gt_labels_3d, preds_dicts, **kwargs):
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            num = torch.clamp(
                reduce_mean(num.detach()), min=1e-4).item()
        ...
        for i in range(num_samples):
            for k in rets[0][i].keys():
                if k == 'bboxes':
                    ...
                    box_type_3d = img_metas[i].get('box_type_3d')
                    if box_type_3d is not None:
                        bboxes = box_type_3d(bboxes, self.bbox_coder.code_size)
                elif k == 'scores':
        ...
        for i in range(num_samples):
            for k in rets[0][i].keys():
                if k == 'bboxes':
                    ...
                    box_type_3d = img_metas[i].get('box_type_3d')
                    if box_type_3d is not None:
                        if isinstance(box_type_3d, list):
                            bboxes = box_type_3d[0](bboxes, self.bbox_coder.code_size)
                        else:
                            bboxes = box_type_3d(bboxes, self.bbox_coder.code_size)
                elif k == 'scores':
                    scores = torch.cat([ret[i][k] for ret in rets])
        ...
        for i, (box_preds, cls_preds, cls_labels) in enumerate(
            ...
            if top_scores.shape[0] != 0:
                ...
                if box_type_3d is not None:
                    if isinstance(box_type_3d, list):
                        boxes_for_nms = xywhr2xyxyr(
                            box_type_3d[0](box_preds[:, :], self.bbox_coder.code_size).bev)
                    else:
                        boxes_for_nms = xywhr2xyxyr(
                            box_type_3d(box_preds[:, :], self.bbox_coder.code_size).bev)
                else:
                    boxes_for_nms = xywhr2xyxyr(
                        torch.stack(
                            [
