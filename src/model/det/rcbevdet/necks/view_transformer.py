...
class LSSViewTransformer(BaseModule):
    ...
    def init_acceleration_v2(self, coor):
        ...
        if ranks_bev is None or ranks_depth is None or ranks_feat is None \
                or interval_starts is None or interval_lengths is None:
            return
    ...
    def get_mlp_input(
        self, sensor2ego, ego2global, intrin, post_rot, post_tran, bda
    ) -> Optional[torch.Tensor]:
        del sensor2ego, ego2global, intrin, post_rot, post_tran, bda
        return None
...
class DepthNet(nn.Module):
    ...
    def __init__(self,
        ...
        if stereo:
            ...
            self.bias = bias
        depth_conv_list: list[nn.Module] = [BasicBlock(depth_conv_input_channels, mid_channels,
    ...
    def calculate_cost_volumn(self, metas):
        ...
        cost_volumn: Optional[torch.Tensor] = None
        wrap_prev = None
        ...
        for fid in range(curr.shape[1] // group_size):
            ...
            if cost_volumn is None:
                cost_volumn = cost_volumn_tmp
            else:
                cost_volumn = cost_volumn + cost_volumn_tmp
        if cost_volumn is None:
            cost_volumn = torch.zeros(
                (B * N, D, H, W), dtype=curr.dtype, device=curr.device
            )
        if self.bias != 0 and wrap_prev is not None:
